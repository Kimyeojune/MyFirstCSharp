...
namespace MyFirstCSharp
{
    public partial class Chap11_BranchingStatment : Form
    {
        ...
        {
            //ORIF문 : 조건 중 하나라도 참일 경우 true로 간주, '이거나'
            // ll 버티컬 바 
            string sValue = txtBS.Text;
        ...
        private void btnPM_Click(object sender, EventArgs e)
        {
            ...
            //입력받은 값을 숫자로 변환 할 수있는지 체크 bool 변수
            bool bFlag = false;

            //입력문자를 숫자로 변환후, iResult 에 담고 성공여부를 bFlag에 담기
            bFlag = int.TryParse(sValue, out iResult);

            //벨리데이션 체크
            //개발자가 프로그램 개발 시 오류 사항이 발생할 수 있는 로직을 
            //예상하고 오류상황이 발생할 수 있는 로직을 처리하기 전에
            //에러 메세지를 사용자에게 전달해 줌으로서
            //시스템 오류를 발생시키지 않으면서 프로그램에 대한 신뢰도를 
            //높이는 개발 방법
            //사용자에게 정확한 사용방법을 알리면서 올바르게 프로그램을 사용
            //할수 있도록 유도
